...
if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($pagamentos as &$p) {
        if ((int)($p['id'] ?? 0) === $id) {
            ...
            $p['inquilino'] = $inquilino;
            $p['apartamento'] = $apartamentoSel;
            $p['valor'] = $valor;
            $p['data'] = $_POST['data'] ?? $p['data'];
...
<main class="main-content">
                    ...
                    <tbody>
                        <?php foreach ($pagamentos as $p): ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn-view" onclick='editarPagamento(<?= htmlspecialchars(json_encode($p)); ?>)' title="Editar"><i class="fas fa-edit"></i></button>
    ...
    <div class="tab-content" id="add">
        <section class="card">
            ...
            <form method="post" action="pagamentos.php?action=create" id="formPagamento">
...
// Função para editar pagamento (prefill form)
function editarPagamento(pag) {
    const form = document.getElementById('formPagamento');
    form.action = 'pagamentos.php?action=edit';
    ...
    idField.value = pag.id;
    const contratoSel = document.getElementById('contrato_id');
    const inqSel = document.getElementById('inquilino');
    const aptSel = document.getElementById('apartamento');
    const valorInp = document.getElementById('valor');
    const dataInp = document.getElementById('data');
    const statusSel = document.getElementById('status');
    if (contratoSel) contratoSel.value = pag.contrato_id || '';
    if (inqSel) inqSel.value = pag.inquilino || '';
    if (aptSel) aptSel.value = pag.apartamento || '';
    if (valorInp) { valorInp.value = parseFloat(pag.valor || 0).toFixed(2); valorInp.readOnly = false; }
    if (dataInp) dataInp.value = pag.data || '';
    if (statusSel) statusSel.value = pag.status || '';
