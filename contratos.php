...
if ($action === 'toggle_status') {
    ...
    foreach ($items as &$it) {
        if ((int)($it['id'] ?? 0) === $id) {
            $current = ($it['status'] ?? 'ativo');
            $it['status'] = $current === 'ativo' ? 'inativo' : 'ativo';
            $changed = true;
            break;
        }
    }
    if ($changed) {
        ...
        // sincronizar status do apartamento associado
        $apartamentos = readData('apartamentos');
        $contratosAll = readData('contratos');
        $altered = null;
        foreach ($contratosAll as $c) { if ((int)($c['id'] ?? 0) === $id) { $altered = $c; break; } }
        if ($altered) {
            $aptNum = $altered['apartamento'] ?? null;
            if ($aptNum) {
                $hasActive = false;
                foreach ($contratosAll as $c) {
                    if (($c['apartamento'] ?? '') === $aptNum && ($c['status'] ?? 'ativo') === 'ativo') { $hasActive = true; break; }
                }
                foreach ($apartamentos as &$ap) {
                    if (($ap['numero'] ?? '') === $aptNum) {
                        $ap['status'] = $hasActive ? 'ocupado' : 'disponivel';
                        break;
                    }
                }
...
if ($action == "create" && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    writeData("contratos", $items);
    // Recalcular status dos apartamentos com base em contratos ativos
    $apartamentosAll = readData('apartamentos');
    $contratosAll = readData('contratos');
    foreach ($apartamentosAll as &$ap) {
        $num = $ap['numero'] ?? null;
        $hasActive = false;
        if ($num) {
            foreach ($contratosAll as $c) {
                if ((($c['apartamento'] ?? '') === $num) && (($c['status'] ?? 'ativo') === 'ativo')) { $hasActive = true; break; }
            }
        }
        $ap['status'] = $hasActive ? 'ocupado' : 'disponivel';
    }
    writeData('apartamentos', $apartamentosAll);
    header("Location: contratos.php");
    exit;
}
...
if ($action == "delete") {
    ...
    // Recalcular status dos apartamentos
    $apartamentosAll = readData('apartamentos');
    $contratosAll = readData('contratos');
    foreach ($apartamentosAll as &$ap) {
        $num = $ap['numero'] ?? null;
...
if ($action == "edit" && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    writeData("contratos", $items);
    // Atualizar pagamentos que referenciam este contrato (sincronizar inquilino/apartamento)
    $editedContract = null;
    foreach ($items as $it) { if ((int)($it['id'] ?? 0) === $id) { $editedContract = $it; break; } }
    if ($editedContract) {
        $pagamentosAll = readData('pagamentos');
        foreach ($pagamentosAll as &$pg) {
            if ((string)($pg['contrato_id'] ?? '') === (string)$id) {
                $pg['inquilino'] = $editedContract['inquilino'] ?? $pg['inquilino'];
                $pg['apartamento'] = $editedContract['apartamento'] ?? $pg['apartamento'];
            }
        }
        writeData('pagamentos', $pagamentosAll);
    }
    // Atualizar status dos apartamentos após criação
    $apartamentosAll = readData('apartamentos');
    $contratosAll = readData('contratos');
    foreach ($apartamentosAll as &$ap) {
        $num = $ap['numero'] ?? null;
                        ...
                        <tr class="<?= ($c['status'] ?? 'ativo') == 'inativo' ? 'tr-inactive' : ''; ?>">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <?php if (($c['status'] ?? 'ativo') === 'ativo'): ?>
                                        <a href="contratos.php?action=toggle_status&id=<?= htmlspecialchars($c["id"]); ?>" class="btn-cancel action-small" onclick="return confirm('Deseja inativar este contrato?')" title="Inativar"><i class="fas fa-power-off"></i></a>
                                    <?php else: ?>
                                        <a href="contratos.php?action=toggle_status&id=<?= htmlspecialchars($c["id"]); ?>" class="btn action-small" onclick="return confirm('Deseja ativar este contrato?')" title="Ativar"><i class="fas fa-check-circle"></i></a>
                                    <?php endif; ?>
...
// lista de apartamentos em JS para preencher valores ao editar
const apartamentos = <?= json_encode($apartamentos); ?>;

function findApt(numero) {
    return apartamentos.find(a => String(a.numero) === String(numero)) || null;
...
function editarContrato(contrato) {
    ...
    form.action = 'contratos.php?action=edit';
    ...
    idField.value = contrato.id;
    form.querySelector('select[name="inquilino"]').value = contrato.inquilino || '';
    form.querySelector('select[name="apartamento"]').value = contrato.apartamento || '';
    const apt = findApt(contrato.apartamento);
    const valorInput = form.querySelector('input[name="valor_aluguel"]');
    if (valorInput) {
        if (apt) {
            valorInput.value = parseFloat(apt.valor || 0).toFixed(2);
        } else {
            valorInput.value = parseFloat(contrato.valor_aluguel || 0).toFixed(2);
        }
        valorInput.readOnly = true;
    }
    form.querySelector('input[name="inicio"]').value = contrato.inicio || '';
    form.querySelector('input[name="fim"]').value = contrato.fim || '';
    form.querySelector('select[name="status"]').value = contrato.status || 'ativo';
    form.querySelector('select[name="tipo_contrato"]').value = contrato.tipo_contrato || '';
    form.querySelector('input[name="arquivo"]').value = contrato.arquivo || '';
