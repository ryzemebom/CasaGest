...
if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($manutencoes as &$m) {
        if ((int)($m['id'] ?? 0) === $id) {
            $m['apartamento'] = $_POST['apartamento'] ?? $m['apartamento'];
            $m['descricao'] = $_POST['descricao'] ?? $m['descricao'];
            $m['data'] = $_POST['data'] ?? $m['data'];
...
<main class="main-content">
                ...
                <tbody>
                    <?php foreach ($manutencoes as $m): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn-view" onclick='editarManutencao(<?= htmlspecialchars(json_encode($m)); ?>)' title="Editar"><i class="fas fa-edit"></i></button>
    ...
    <div class="tab-content" id="add">
        <section class="card">
            ...
            <form method="post" action="manutencoes.php?action=create" id="formManutencao">
...
function editarManutencao(m) {
    ...
    form.action = 'manutencoes.php?action=edit';
    ...
    idField.value = m.id;
    form.querySelector('select[name="apartamento"]').value = m.apartamento || '';
    form.querySelector('textarea[name="descricao"]').value = m.descricao || '';
    form.querySelector('input[name="data"]').value = m.data || '';
    form.querySelector('select[name="status"]').value = m.status || 'pendente';
